...
TTripleSliderDemo::TTripleSliderDemo() : TGMainFrame(gClient->GetRoot(), 100, 100)
{
   ...
   T->Draw("L.prl1.e:L.prl2.e>>h2(500,0,3000,500,0,3000)","","colz");
...
void TTripleSliderDemo::DoSlider()
{
   ...
   Double_t ymax = 100000;
   Double_t vzmin = -0.08, vzmax = 0.08;
   Double_t thmin = -0.03, thmax = 0.05;
   Double_t phmin = 0., phmax = 0.;
   ...
   //TCut ct_trigger = "(DR.evtypebits&1<<3)==1<<3";

   pad1->cd(1);
   //Draw the Cherenkov spectrum.
   T->Draw("L.cer.asum_c>>h1(500,0,3000)",ct_1tr&&ct_vz&&ct_th);
   h1->GetYaxis()->SetTitle("Counts");
   h1->GetXaxis()->SetTitle("L.cer.asum_c");
   h1->SetMaximum(ymax);
   h1->GetYaxis()->SetTitleOffset(1.3);
   ...
   Double_t epmin = 0.8;
   TCut ct2 = Form("(L.prl1.e+L.prl2.e)/(1000*L.tr.p)>%f",epmin);

   //Draw the shower and preshower with the Cherenkov cut applied.
   T->Draw("L.prl1.e:L.prl2.e>>h2(500,0,3000,500,0,3000)",ct1&&ct_1tr&&ct_vz&&ct_th,"colz");
   h2->GetYaxis()->SetTitle("L.prl1.e");
   h2->GetXaxis()->SetTitle("L.prl2.e");
   h2->GetYaxis()->SetTitleOffset(1.55);
...
void Cherenkov_Cut_3He(Int_t run)
{
//============  Reading the Rootfile =======================//
  ...
  //Double_t shmin = 0.8;
  //TCut ct2 = Form("(L.prl1.e+L.prl2.e)/(1000*L.tr.p)>%f",shmin);
